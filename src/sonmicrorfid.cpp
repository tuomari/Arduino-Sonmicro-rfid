...
static const byte MSG_PREFIX[] = { 0xFF, 0x00 };
...
bool SonmicroRfid::authenticate() {
	...
	byte command[9] = { 0x85, _block, _keytype };
...
bool SonmicroRfid::decrement(const int32_t decvalue) {
	...
	byte cmd[] = { 0x8E, _block, 0x00, 0x00, 0x00, 0x00 };
...
bool SonmicroRfid::increment(const int32_t incvalue) {
	...
	byte cmd[] = { 0x8D, _block, 0x00, 0x00, 0x00, 0x00 };
...
void SonmicroRfid::getUidAsChar(char* str) const {
	...
	int uidlen = 4;
	if (_cardtype == ULTRALIGHT) {
		uidlen = 7;
	}
	hexToString(_carduid, str, uidlen);
	str[uidlen * 2] = '\0';
